...
To disable per-key statistics tracking, set env var `ENABLE_PER_KEY_STATS` to `false`. When disabled (default), all stats are aggregated under an "all" key to prevent memory leaks from unbounded key growth.
